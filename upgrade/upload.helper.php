...
class upload_helper {
    ...
    private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                //$metas['extension'] = substr(strrchr(strtolower($field['name'][$i]), "."),1);substr(strrchr($field['name'][$i]),"."),1)
    ...
    private function generateFileName($conf, $file){
        ...
        if(preg_match('/ /',$string)){$string = strtolower(str_replace(" ","-",$string));}else{$string = strtolower($string);}
        return $string;
    }
