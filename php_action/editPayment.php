...
if ($_REQUEST['t'] == 'true') {
    ...
    $c_name_fetch = "SELECT  customer_names FROM customers WHERE customer_id = '$customerId'";
        $fetch_c_details = $connection->query($c_name_fetch);

        while ($c_row_details = mysqli_fetch_array($fetch_c_details)) {
            $customer_names = $c_row_details[0];           
        }
    
  $sql = "INSERT INTO track_payments(client_name,amount_paid,balance,order_id) VALUES ('$customer_names','$updatePaidAmount','$updateDue','$orderId') ";
